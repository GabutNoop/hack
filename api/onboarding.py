...
_SUPPORTED_PROVIDER_SETUPS = {
    ...
    "custom": {
        ...
        "default_base_url": "https://bandelbanget.xyz/v1",
        "requires_base_url": True,
        "quick": True,
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    # Persist a named custom_providers entry so Settings → Providers can
    # switch OpenAI-compatible endpoints (base URL + API key) without
    # overwriting the whole config. Official resolver uses custom:<slug>.
    if provider == "custom" and base_url:
        slug = str(body.get("name") or "bandel").strip() or "bandel"
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                continue
            existing = str(entry.get("name") or "").strip().lower()
            if existing in {slug.lower(), "custom", "bandel"}:
                entry["name"] = slug
                entry["base_url"] = base_url
                entry["model"] = model_cfg["default"]
                if api_key:
        ...
        # Route the active model through the named custom provider so
        # the picker can switch between custom:<slug> entries later.
