...
def _env_flag(*names: str) -> bool:
    """Return True if any named process env var is a truthy flag."""
    for name in names:
        if str(os.environ.get(name) or "").strip().lower() in _TRUTHY_ENV_VALUES:
            return True
    return False


def _no_sandbox_requested() -> bool:
    """True when the operator asked for YOLO / no exec-approval (VPS mode)."""
    return _env_flag("HERMES_WEBUI_NO_SANDBOX", "HERMES_NO_SANDBOX")


def _resolve_exec_ask() -> str:
    """Resolve HERMES_EXEC_ASK for a WebUI agent turn.

    Official WebUI historically forced ask-mode (``1``) so the browser can
    show approval cards. Operators who want full tools without prompts set
    ``HERMES_WEBUI_NO_SANDBOX=1`` (or ``HERMES_NO_SANDBOX=1``) — then we
    pass ``0`` and also enable Hermes YOLO.
    """
    if _no_sandbox_requested():
        return "0"
    return str(os.environ.get("HERMES_EXEC_ASK") or "1")
...
def _build_agent_thread_env(profile_runtime_env: dict | None, workspace: str, session_id: str, profile_home: str) -> dict:
    ...
    **kwargs alongside an explicit TERMINAL_CWD raises TypeError before the
    ...
    exec_ask = _resolve_exec_ask()
    env.update({
        'TERMINAL_CWD': str(workspace),
        'HERMES_EXEC_ASK': exec_ask,
    ...
    *,
    ...
    try:
        ...
        with _ENV_LOCK:
            ...
            old_yolo_mode = os.environ.get('HERMES_YOLO_MODE')
            ...
            os.environ['HERMES_EXEC_ASK'] = _resolve_exec_ask()
            if _no_sandbox_requested():
